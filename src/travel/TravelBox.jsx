...
const TravelBox = (props) => {
    ...
    const introInfoStyling = 'sm:col-start-1 sm:col-end-2';
    const changePhotoStyling = `${isDark ? `bg-black` : `bg-bg-gray-100`} sm:w-10 sm:mx-5 w-10 row-start-2 row-end-3 sm:row-start-1 sm:row-end-2 col-start-1 col-end-2 sm:col-start-10 sm:col-end-11`;
            ...
            <div className='travel-year-introduction my-8'>
                <div className='travel-year-intro-info'>
                    <div className={`col-span-1 flex flex-col justify-between sm:grid sm:grid-cols-2`}>
                        <div className={`${introInfoStyling} sm:row-start-1 sm:row-end-2 travel-year mb-5`}><p className='text-customBlue text-2xl font-bold'>{props.year}</p></div>
                        <div className={`${introInfoStyling} sm:row-start-2 sm:row-end-3 sm:flex sm:items-center travel-title text-3xl mb-5`}><h4>{props.title}</h4></div>
                        ...
                        <div className={`${introInfoStyling} sm:row-start-3 sm:row-end-4 sm:items-start flex flex-wrap travel-countries my-5`}>
                        ...
                        <div className={`${introInfoStyling} sm:row-start-4 sm:row-end-5 sm:flex sm:items-end travel-extend-box`} style={{ visibility: !isLastStory ? 'block' : 'hidden' }}>
                            <div className={`read-more-less ${isDark ? `border-white` : `border-black`} border-2 w-32 pl-3`} onClick={() => props.toggleVisible(props.index)}>
                                <span>{props.isVisible ? 'READ LESS' : 'READ MORE'}</span><span>{props.isVisible ? '▲' : '▼'}</span>
                            </div>
                        </div>
                    </div>
            ...
            <div className={`year-photos ${isDark ? `bg-black` : `bg-gray-100`} mb-10 flex justify-center`} ref={photoRef} style={{ display: props.isVisible && props.photos.length > 0 ? 'flex' : 'none' }}>
                {props.photos.length > 0 && (
                    <div className='travel-photo grid grid-cols-1 sm:grid-cols-10 flex items-center'>
                        <svg className={`${changePhotoStyling} justify-self-start`} data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"  style={{ display: props.photos.length === 1 ? 'none' : 'block' }} onClick={prevPhoto}>
                        ...
                        <svg className={`${changePhotoStyling} justify-self-end`} data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style={{ display: props.photos.length === 1 ? 'none' : 'block' }} onClick={nextPhoto}>
